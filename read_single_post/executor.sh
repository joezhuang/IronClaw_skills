...
async def run_read(target_url):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Global engine context stealth mapping
            ...
            try:
                ...
                if ext_url and "x.com" not in ext_url and "twitter.com" not in ext_url:
                    ...
                    try:
                        ...
                        # 🌟 CLOUDFLARE DETECTOR & JINA READER BYPASS
                        page_title = await new_tab.title()
                        content_check = await new_tab.evaluate("() => document.body.innerText")
                        
                        if "Just a moment" in page_title or "Cloudflare" in page_title or "Performance and Security by Cloudflare" in content_check:
                            print("🛡️ Cloudflare blocked us. Routing through Jina AI Reader...", file=sys.stderr)
                            
                            # Prepend the Jina Reader API to the target URL
                            jina_url = f"https://r.jina.ai/{ext_url}"
                            
                            await new_tab.goto(jina_url, timeout=20000, wait_until="domcontentloaded")
                            await asyncio.sleep(random.uniform(3.0, 5.0))
                            
                            # Jina returns clean text automatically wrapped in the body
                            article_data = await new_tab.evaluate("() => document.body.innerText")
                            
                            # Ultimate Failsafe: If even Jina fails, block the garbage text so the LLM doesn't hallucinate
                            if "Performance and Security by Cloudflare" in article_data or "Just a moment" in article_data:
                                result["article_context"] = "⚠️ ARTICLE BLOCKED BY CLOUDFLARE/PAYWALL. Rely ONLY on the RAW POST CONTENT above to draft your reply."
                            else:
                                result["article_context"] = article_data[:6000]
                        else:
                            # Standard extraction for normal, unblocked websites
