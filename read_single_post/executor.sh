                    ...
                    if ext_url and "x.com" not in ext_url and "twitter.com" not in ext_url:
                        ...
                        try:
                            ...
                            article_data = await new_tab.evaluate('''() => {
                                ...
                                // 🛑 THE MECHANICAL FILTER: Target main content wrappers first
                                let container = document.querySelector('article') || 
                                                document.querySelector('main') || 
                                                document.querySelector('.article-body') || 
                                                document.querySelector('.post-content') || 
                                                document.querySelector('.entry-content') || 
                                                document.body; // Fallback only if semantic tags fail
                                
                                // Grab paragraphs ONLY from the isolated container
                                let pTexts = Array.from(container.querySelectorAll('p'))
                                    .map(p => p.innerText.trim())
                                    .filter(text => text.length > 50) // Drops tiny nav links/buttons
                                    .join('\\n\\n');
                                    
                                return `TITLE: ${title}\\nDESC: ${desc}\\n\\nBODY:\\n${pTexts}`.substring(0, 4000);
                            }''')
