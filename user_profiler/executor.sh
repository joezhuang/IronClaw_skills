...
try:
    ...
    # 🎯 FIX: SKILL_NAME is evaluated safely here after args is completely defined!
    SKILL_NAME = args.get("skill_scope", "user_profiler")
    ...
    if action == "save_preference":
        ...
        if not facts:
            ...
            for fact in facts:
                ...
                if closest and closest[1] < 0.10:
                    ...
                    # Change it to this:
                    cursor.execute('UPDATE facts SET fact = ?, created_at = CURRENT_TIMESTAMP WHERE id = ?', (fact, old_id))
        ...
        if query:
            ...
            cursor.execute('''
                SELECT facts.fact, facts.created_at 
                FROM vec_memory
                LEFT JOIN facts ON facts.id = vec_memory.rowid
...
# # Change it to this (defaults to user_profiler if not provided):
# SKILL_NAME = args.get("skill_scope", "user_profiler")
