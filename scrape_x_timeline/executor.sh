...
async def run_scrape():
    try:
        ...
        async with async_playwright() as p:
            ...
            # Humanized variable scrolling
            scroll_loops = random.randint(3, 5)
            for _ in range(scroll_loops):
                scroll_distance = random.randint(600, 1050)
                await page.mouse.wheel(0, scroll_distance)
                
                if random.random() > 0.4:
                    await page.mouse.move(random.randint(150, 750), random.randint(150, 650))
            ...
            if valid_posts_found == 0:
                ...
                print("You MUST immediately execute the `read_single_post` tool. You must pass ONLY the `target_url` parameter for the post you select.")
                print("="*40)
